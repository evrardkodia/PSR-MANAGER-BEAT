...
def extract_section(input_path, output_path, section_name):
    try:
        ...
        if end_tick is None:
            end_tick = mid.length * ticks_per_beat * 2  # estimation généreuse
        ...
        def ticks_to_seconds(ticks, tempo, tpq):
            return round((ticks * tempo) / (tpq * 1_000_000), 3)

        start_sec = ticks_to_seconds(start_tick, tempo, ticks_per_beat)
        end_sec = ticks_to_seconds(end_tick, tempo, ticks_per_beat)
        duration = round(end_sec - start_sec, 3)

        print(f"{start_sec};{duration}")  # <- capté par Node.js
        log_debug(f"Start: {start_sec}s | Duration: {duration}s")

        return True, start_sec, duration
...
if __name__ == "__main__":
    ...
    success, _, _ = extract_section(input_path, output_path, section_name)
    if not success:
        sys.exit(1)
